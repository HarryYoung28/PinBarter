...
export default function MakeOfferForm({ listing, onClose, onSuccess }) {
    ...
    // state
    // collection of available pins the user can offer
    const [collection, setCollection] = useState([])
    const [loading, setLoading] = useState(true)

    // up to 4 pin slots, each holds a selected pin id or null
    const [selectedPins, setSelectedPins] = useState([null])
    const [submitting, setSubmitting] = useState(false)
    const [error, setError] = useState("")

    // shows a warning if the user is offering more than the pin's credit value
    const [overValueWarning, setOverValueWarning] = useState(false)

    // fetch the user's available pins on mount
    // filters out pins already listed or committed to pending trades
    useEffect(() => {
        async function fetchAvailablePins() {
            ...
            // remove pins that are already listed or in a pending trade
    ...
    // calculate the total credit value of all selected pins
    const totalCredits = selectedPins.reduce((sum, pinId) => {
        const pin = collection.find(p => p.id === pinId)
        return sum + (pin ? pin.credits : 0)
    }, 0)

    // calculate minimum credits required based on listing flexibility
    ...
    // functions
    // updates the selected pin at a specific slot index
    ...
    // adds a new empty pin slot up to a maximum of 4
    function addSlot() {
    ...
    // removes a pin slot by index, always keeps at least one slot
    function removeSlot(index) {
        const updated = selectedPins.filter((_, i) => i !== index)
        setSelectedPins(updated.length === 0 ? [null] : updated)
    }

    // returns pins available for a specific slot
    // excludes pins already selected in other slots
    ...
    // handles offer submission with validation
    async function handleSubmit() {
        ...
        // if the user is offering more than the pin value, show a warning first
            ...
            <div className="
                ...
                <p className="text-sm text-gray-500 dark:text-gray-300 mb-1">
                    Offering for: <span className="
                        ...
                        {listing.pin.name}
                    </span> ({listing.pin.credits} {listing.pin.credits === 1 ? "credit" : "credits"})
                </p>
                <p className="text-sm text-gray-500 dark:text-gray-300 mb-4">
                    Minimum offer: <span className="font-medium">{minimumCredits} credits</span>
                </p>

                {/* loading state while collection is being fetched */}
                {loading && (
                    <p className="
                        text-sm
                        text-gray-500
                ...
                {/* empty state when user has no available pins to offer */}
                {!loading && collection.length === 0 && (
                    <p className="text-sm text-gray-500 dark:text-gray-300 text-center py-4">
                        You have no pins available to offer!
                    </p>
                )}
                ...
                {!loading && collection.length > 0 && (
                    <>
                        <div className="flex flex-col gap-3 mb-4">
                            {selectedPins.map((selectedPinId, index) => (
                                <div key={index} className="flex items-center gap-2">
                                    ...
                                    {/* remove slot button only shows when there is more than one slot */}
                        ...
                        {/* add another pin slot button, hidden when 4 slots are already showing */}
                        ...
                        {/* credit total summary, turns green when minimum is met */}
                        <div className="border-t border-gray-200 dark:border-gray-600 pt-3 mb-4">
                            <p className="text-sm text-gray-600 dark:text-gray-300">
                                Your offer total:{" "}
                                <span
                                    data-testid="offer-total"
                                    className={`font-bold ${meetsMinimum ? 'text-green-500' : 'text-red-500'}`}>
                ...
                {/* over value warning shown when user offers more than the pin is worth */}
                {overValueWarning && (
                    <div className="
                        bg-yellow-50
